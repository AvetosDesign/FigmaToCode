...
import { assignUniqueSlugs, toPresetSlug, toSlug } from "./slugify";
...
describe("toPresetSlug", () => {
  it("inserts a hyphen at letter->digit and digit->letter boundaries after slugifying", () => {
    // A hash-like name should split at letter/digit boundaries so it
...
describe("assignUniqueSlugs", () => {
  ...
  it("appends -2, -3, ... on collision, first-seen-wins order", () => {
    expect(assignUniqueSlugs(["Primary", "Primary", "Primary"])).toEqual([
      "primary",
      "primary-2",
      "primary-3",
    ]);
  ...
  it("only disambiguates names that actually collide after slugifying", () => {
    expect(assignUniqueSlugs(["Primary", "Secondary", "Primary"])).toEqual([
      "primary",
      "secondary",
      "primary-2",
    ]);
  ...
  it("uses the provided slugFn instead of the default toSlug", () => {
    expect(assignUniqueSlugs(["a1b2", "a1b2"], toPresetSlug)).toEqual([
      "a-1-b-2",
      "a-1-b-2-2",
    ]);
  });

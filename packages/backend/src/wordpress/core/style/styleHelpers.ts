 ...
 * way, only where the caller puts the result differs (almost everything
 * now goes into a generated stylesheet rule, not inline).
 */
 ...
 * absolute position). This has no attrs-JSON counterpart at all (an
 * earlier `layoutToFlexAttrs`/`layout` JSON attr was removed —
 * WordPress's own layout block-support generates its own container class
 * + stylesheet from that attr, which we were never reproducing, so it
 * could never validate; simplest fix is to not claim it as an attr in the
 * first place). This is purely a CSS-rule-body builder now.
 *
 * Additions beyond the original port:
 ...
 *   ancestor for.
 * - `paintOrder` (second param here) → `z-index: {paintOrder}`, only
 *   alongside `position: absolute` (z-index is a no-op on
 ...
 *   Part extraction: once a node is pulled out of its original
 *   `children[]` and rendered as a completely separate sibling file, DOM
 *   order alone can no longer reproduce whether it painted above or
 *   below content that stayed behind — explicit z-index on both sides
 *   (this, and the template-part inclusion's own rule — see
...
export const layoutToDeclarations = (layout: DesignBundleLayout): string => {
  ...
  // Figma's Auto Layout has no margin concept at all — every bit of
  ...
  // nonzero user-agent default (found on `core/paragraph` specifically),
  // and WordPress's block-supports layout classes
  // (`.is-layout-flow`/`.is-layout-flex`) contribute their own default
  // spacing independent of any one block's actual `layoutMode`. Rather
  // than continuing to chase each individual source of unwanted margin
  // one at a time, force it to zero unconditionally, on every node this
  // function styles — nothing in the Figma model ever wants a margin, so
  // nothing generated here should ever have one apply by accident. A
  // node's own `padding`/parent's `gap` declarations (already handled
  ...
  if (layout.mode !== "NONE") {
    ...
    // Figma's Auto Layout wrap — CSS's literal `flex-wrap: wrap`
    ...
    declarations.push([
      "justify-content",
      primaryAxisAlignToCss(layout.primaryAxisAlign),
    ]);
    declarations.push([
      "align-items",
      counterAxisAlignToCss(layout.counterAxisAlign),
    ]);
...
/**
 * A CSS-optimization step: a node's absolute-position declarations
 * (`left`/`top`/`z-index`), split out of `layoutToDeclarations` so they
 * can go into their own, never-deduped rule (`addPositionRule`,
 * `core/style/stylesheet.ts`) instead of being folded into the same
 * declaration string as everything else describing a node's appearance.
 * Two nodes with the exact same "look" (flex/gap/padding/color/border/
 * etc.) very often sit at *different* canvas coordinates — before this
 * split, that alone was enough to make their generated rules look unique
 * even though the actual visual styling was identical, defeating the
 * dedup for exactly the cases where it would matter most (a repeated
 * card, a repeated icon+label row). Position itself is still inherently
 * per-node (two nodes are essentially never at the same coordinates) and
 * is never expected to be shared — see `addPositionRule`'s own doc
 * comment.
 *
 ...
 * in this file.
 */
export const layoutPositionToDeclarations = (
  ...
  const declarations: Array<[string, string | undefined]> = [];
  // Found via the footer Template Part rendering as a "staircase" of
  // increasingly offset elements in the WordPress block editor (both the
  // Page/Post editor and the Site Editor's template view) — confirmed
  // correct on the front end, broken only in-editor. Root cause,
  // confirmed directly via dev-tools computed-style inspection:
  // `add_theme_support('editor-styles')`/`add_editor_style` wraps every
  // rule this project generates in `:where(.editor-styles-wrapper) ...`
  // when loading it into the editor — `:where()` always has zero
  // specificity by CSS spec, regardless of what's nested inside it, so
  // our selector's real specificity there is just the bare `.wpfg-{id}`
  // class. Gutenberg's own editor-canvas CSS unconditionally sets
  // `position: relative` on every block via a higher-specificity
  // compound selector (`.block-editor-block-list__layout
  // .block-editor-block-list__block`, two chained classes) — it wins the
  // cascade and silently downgrades every absolutely-positioned element
  // back to `relative`, which then reinterprets our `left`/`top` (meant
  // as absolute coordinates) as relative offsets stacked on top of
  // normal flow — explains the staircase exactly. Every other
  // declaration this function generates was confirmed still winning
  // correctly in the editor (checked directly in dev tools) — only
  // `position` needed defending, so `!important` is applied surgically
  // to just this one declaration rather than the whole rule.
  ...
  if (paintOrder !== undefined)
    declarations.push(["z-index", String(paintOrder)]);

  return buildInlineStyle(declarations);
};

/**
 * Combines a fill's plain 6-digit `#RRGGBB` hex with its own
 * separately-captured `opacity` (Stage 1 already collapsed Figma's
 * `color.a` and `paint.opacity` into this one number) into an 8-digit
 * `#RRGGBBAA` — real, modern CSS, not a rgba() rewrite, so it slots into
 * every existing `background-color`/`color` declaration unchanged.
...
/**
 * Converts a LINEAR/RADIAL/ANGULAR `DesignBundleGradient` into a real CSS
 * `linear-gradient()`/`radial-gradient()`/`conic-gradient()` function
 * value. Ported from this fork's own pre-existing
 * `html/builderImpl/htmlColor.ts` gradient math (`htmlLinearGradient`/
 * `htmlRadialGradient`/`htmlAngularGradient`), adapted to this schema's
 * already-alpha-combined 8-digit stop hexes instead of separate
 * color+opacity. Figma's raw `gradientHandlePositions` geometry
 * (normalized 0-1, unconverted since Stage 1 — see
 ...
 * since `DesignBundleFill.gradient` is undefined for that kind.
 */
export const gradientToCss = (gradient: DesignBundleGradient): string => {
  const stopsAt = (positionMultiplier: number, unit: string): string =>
    gradient.stops
      .map(
        (stop) =>
          `${stop.hex} ${(stop.position * positionMultiplier).toFixed(0)}${unit}`,
      )
      .join(", ");
 ...
 * A GRADIENT fill with real LINEAR/RADIAL/ANGULAR geometry renders as a
 * `background-image` gradient function via `gradientToCss`;
 ...
 * `has-{slug}-background-color`) — no need to duplicate the same color in
 * our own stylesheet too (gradients never take this path, since they
 * never resolve a `variableRef`). When `skipBackground` is true, a solid
 * fill's `opacity` (if any) is simply dropped along with the color — a WP
 * preset color is always fully opaque, there's no mechanism to apply
 * per-instance alpha to one.
 *
 * `style.opacity` (the *node's own* layer opacity, as opposed to any one
 * fill's) becomes a plain CSS `opacity` declaration — affects this node's
 * entire rendered result (background, border, and every descendant) as a
 * single compositing group, deliberately not folded into the
 * background-color's alpha channel above (the two represent genuinely
 * different things: a fill's own alpha vs. the whole node's).
 */
/**
 ...
 */
export const effectsToDeclarations = (
  effects: DesignBundleEffect[],
): Array<[string, string | undefined]> => {
  const shadows = effects
    .filter(
      (e) => (e.type === "DROP_SHADOW" || e.type === "INNER_SHADOW") && e.hex,
    )
    .map((e) => {
  ...
  const layerBlur = effects.find(
    (e) => e.type === "LAYER_BLUR" && typeof e.blur === "number",
  );
  const backgroundBlur = effects.find(
    (e) => e.type === "BACKGROUND_BLUR" && typeof e.blur === "number",
  );
...
export const nodeStyleToDeclarations = (
  ...
  if (paintableFill?.hex && !skipBackground) {
    if (paintableFill.gradient) {
      declarations.push([
        "background-image",
        gradientToCss(paintableFill.gradient),
      ]);
    } else {
      declarations.push([
        "background-color",
        withAlpha(paintableFill.hex, paintableFill.opacity),
      ]);
    }
  }
  ...
  if (stroke?.hex)
    declarations.push(["border", `${stroke.weight}px solid ${stroke.hex}`]);

  if (style.cornerRadius)
    declarations.push(["border-radius", `${style.cornerRadius}px`]);

  if (style.opacity !== undefined)
    declarations.push(["opacity", String(style.opacity)]);

  // The node's own blend mode -> CSS `mix-blend-mode`. Already a real CSS
  // keyword by the time it reaches here (Stage 1's `nodeBlendMode` did the
  // Figma-name -> CSS-name conversion), so this is a direct pass-through,
  // same shape as `opacity` just above.
  if (style.blendMode !== undefined)
    declarations.push(["mix-blend-mode", style.blendMode]);
...
/**
 * A real, structural gap surfaced by a real-WordPress screenshot — every
 * generated Figma text run rendered as the browser's default serif font,
 * uniformly, everywhere. Root cause: Stage 1 only ever exports *image*
 * assets (`raster`/`vector` — see `designBundleTree.ts`), never font
 * files, and the mapper's font-family declaration was a bare name with no
 * fallback (`font-family: Inter`) — when "Inter" isn't an installed
 * system font (the common case) and nothing on the page loads it (no
 * `@font-face`, no Google Fonts link), the browser has nowhere to fall
 * back to but its own document default, which is commonly serif. This is
 * a stopgap, not a fix for the underlying gap: it does **not** make
 * Figma's actual font (e.g. real Inter) render — actually loading real
 * font files (self-hosted via theme.json `fontFace`, or a Google Fonts
 * `<link>`) was a separate, bigger decision deferred at the time
 * (network/hosting tradeoff), and was in fact later implemented — see
 * `googleFonts.ts`. All this does is add a generic CSS fallback family so
 * text renders in *some* reasonable default (the browser's UI sans-serif,
 * matching what most product/UI design-system fonts — including Inter —
 * actually are) instead of silently falling through to serif with zero
 * declaration at all.
 *
...
export const fontFamilyDeclaration = (fontFamily: string): string => {
  ...
  const generic = MONOSPACE_FAMILY_HINTS.some((hint) => lower.includes(hint))
    ? "monospace"
    : SERIF_FAMILY_HINTS.some((hint) => lower.includes(hint)) &&
        !lower.includes("sans")
      ? "serif"
      : "sans-serif";
